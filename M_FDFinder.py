...
if __name__ == "__main__": 
    ...
    #f0 = open(Path+Input+"_test.maf", 'r')
    
    f0 = open(Path+Input+".maf", 'r')                  ###
    f3 = open(Path + ChrNameFile + '.txt', 'r')
    
    #fLog = open(LogPath + 'M_FDFLog.txt','w')
    #f4 = open(Path + 'PreFDLocibyDic_Val.txt', 'w')
    #f7 = open(Path + 'VGPFDLocibyDic_Val.txt', 'w')
    #f4 = open(Path + 'CactusXPurgedupFD.txt', 'w')
    #f9 = open(Path + 'DepthDicbyType_Val.dic', 'wb')
    #f6 = open(Path+Input+"TypeDicBinary_Val", 'wb')
...
###################################################################################################
### [ContigDic Clustering]:Suppressed                                               ###############
def FDAdjacency(ContigDic):
    InContigDic = ContigDic
    FDAdjacencyList = []
    for i in InContigDic:
        Chr, LociList = i, InContigDic[i].keys()
        Index = 0
        for j in LociList:
            if Index == 0:  PreEnd = int(j.split('-')[0])
            else:
                Start = int(j.split(':')[1].split('-')[0])
                FDAdjacencyList.append(Start - PreEnd)
    return FDAdjacencyList # List to distribution


def Clustering4TD(BlockDic, Loci, BlockNoList, Cutoff):
    MChr, MStart, MEnd = Loci[0], int(Loci[1]), int(Loci[2])
    OverlapBlockList = []
    for i in BlockNoList:
        BlockNo, DL = i, BlockDic[i]
        FChr, FStart, FEnd = DL[0], int(DL[1]), int(DL[2])
        if FChr == MChr and FStart <= MEnd and MStart <= FEnd: 
            OverlapBlockList.append(BlockNo)

    TChrs, TDPos = [], []
    for i in OverlapBlockList:
        BlockNo, DL = i, BlockDic[i]
        TStart, TEnd, TChr = int(DL[3]), int(DL[4]), DL[5]
        TChrs.append(TChr)
        TDPos.append([TStart, TEnd])
    if len(set(TChrs)) == 1: pass
    else: return 'Diff', ['Non','Non']
    TDPos.sort()
    Index = 0
    LargeEnd = 0
    for i in TDPos:
        if Index == 0: Index += 1
        else: # [1,3], [1,10], [4,6], [8,9], [10,11]
            PreEnd, Start, End = TDPos[Index - 1][1], i[0], i[1]
            if LargeEnd < PreEnd: LargeEnd = PreEnd
            if Start - LargeEnd - 1 <= MergeCutoff: pass
            else: return 'Diff', ['Non','Non']
            Index += 1
    if LargeEnd < End: LargeEnd = End  #All clustered
    return 'Clustered', [TDPos[0][0], LargeEnd]


def ContigDicFDInnerCluster(ContigDic, BlockDic, MergeCutoff): # {Chr:{Loci1:{'FDPos':[Start, End], 'FDBlock':[BLockNo1, BLockNo2], 'CFDType':['Whole' or 'Edge','Central'..]'},Loci2:{}} # FDPos가 FDPos-1과 10bp cutoff? -> True에선? 
    ...
    for i in InContigDic:
        ...
        for j in LociDic:
            ...
            FDPosList, FDBlockList= DD['FDPos'], DD['FDBlock']
            FDPosList.sort()
            ClusterDic, ClusteredPos, ClusterNo = {}, [], 0
            for k in FDPosList:
                Start, End = k[0], k[1]
                if len(Clustered) == 0:
                    TCluster = Clustering4TD(InBlockDic, [Chr, Start, End], FDBlockList, MergeCutoff)
                    Clustered = [[Start, End], [TCluster[1][0], TCluster[1][1]]]
                else:
                    TCluster = Clustering4TD(InBlockDic, [Chr, Clustered[0], End], FDBlockList, MergeCutoff)
                    if (Start - Clustred[0][1] - 1) <= MergeCutoff and TCluster[0] == 'Clustered': 
                        Clustered[0][1] = End
                        Clustered[1] = TCluster[1]
                    else:
                        ClusterDic[str(ClusterNo)] = Clustered
                        TCluster = Clustering4TD(InBlockDic, [Chr, Start, End], FDBlockList, MergeCutoff)
                        Clustered = [[Start, End], [TCluster[1][0], TCluster[1][1]]]
                        ClusterNo += 1
            ClusterDic[str(ClusterNo)] = Clustered
            InContigDic[Chr][Loci]['ClusterDic'] = ClusterDic
    return InContigDic


### [Others]
def ContigDicFDInnerClusterbyDist(ContigDic, MergeCutoff):
    InContigDic = ContigDic
    for i in ContigDic:
        Chr, LociDic = i, InContigDic[i]
        for j in LociDic:
            ...
            FDPosList, FDBlockList= DD['FDPos'], DD['FDBlock']
            FDPosList.sort()
            ClusterDic, ClusteredPos, ClusterNo = {}, [], 1
            for k in FDPosList: #Non-overlapped Regions
                Start, End = k[0], k[1]
                if len(Clustered) == 0:  Clustered = [Start, End] # Initialization
                else:
                    if (Start - Clustered[0][1] - 1) <= MergeCutoff:  
                        Clustered[0][1] = End
                    else: 
                        ClusterDic['Cluster ' + str(ClusterNo)] = Clustered
                        Clustered = [Start, End] # re-initialization
                        ClusterNo += 1
            ClusterDic['Cluster ' + str(ClusterNo)] = Clustered
            InContigDic[Chr][Loci]['ClusterDic'] = ClusterDic
    return InContigDic

def Contig2FDTMerge(CFDChr, CFDLoci, BlockList): # 1. Find FD loci. 2. Check overlap FD. 3. T Overlap merge or seperatly do ??: TD region of Overlap FD region parsing
    CFDStart, CFDEnd = CFDLoci[0], CFDLoci[1]
    OverlapBlock = []
    OverlapTChr, OverlapTPos = [], []
    for i in BlockList:
        #BlockNo, FChrm FStart, FEnd, TChr, TStart, TEnd =  
        if CFDStart <= FEnd and FStart <= CFDEnd:
            OverlapBlock.append(BlockNo)
            OverlapTChr.append(TChr)
            OverlapTPos.append([TStart, TEnd])
    if len(set(OverlapTChr)) == 1: MergedTList = OverlapMerge(OverlapTPos)
    else:  MergedTList = OverlapTpos
    return OverlapBlock, MergedTList #MergedTList > 1: 따로계산
